...
export const useTable = <T, K, L extends AxiosConfig = AxiosConfig>(
	...
	const methods: {
		...
		getSelection: () => Promise<K[]>
		delList: (ids: string[] | number[], multipart: boolean, message?: boolean) => Promise<void>
	} = {
		getList: async () => {
			tableObject.loading = true
			const res = await config?.getListApi(unref(paramsObj) as unknown as L)
				.catch(() => {})
				.finally(() => {
